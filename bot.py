...
def btn(uid, key):
    ...
    data = {
        ...
        "de": {
            BTN_CREATE: "🍻 Profil erstellen",
            BTN_PROFILE: "👤 Profil",
            BTN_NEAR: "👀 In der Nähe",
            BTN_MAP: "🗺 Karte",
...
def main_menu_kb(uid):
    ...
    if has_active_post(uid):
        ...
        buttons = [
            [KeyboardButton(text=btn(uid, BTN_PROFILE))],
            [KeyboardButton(text=btn(uid, BTN_NEAR))],
            [
                KeyboardButton(
                    text=btn(uid, BTN_MAP),
                    web_app=WebAppInfo(url=WEBAPP_URL)
                )
            ],
            [KeyboardButton(text=btn(uid, BTN_SETTINGS))]
        ]

    else:

        buttons = [
            [KeyboardButton(text=btn(uid, BTN_CREATE))],
            [KeyboardButton(text=btn(uid, BTN_NEAR))],
            [
                KeyboardButton(
                    text=btn(uid, BTN_MAP),
...
@dp.message(lambda m: m.from_user.id in user_step and m.text is not None)
...
async def meet_request(call: CallbackQuery):
    ...
    exists = sb.table("matches") \
        ...
        .or_(
            ...
            f"and(user1.eq.{uid},user2.eq.{other})"
        )
        .execute().data
